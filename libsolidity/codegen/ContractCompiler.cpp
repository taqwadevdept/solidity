	...
	identifierAccess.generateCode = [&](yul::Identifier const& _identifier, yul::IdentifierContext _context, yul::AbstractAssembly& _assembly)
	{
		...
		if (_context == yul::IdentifierContext::RValue)
		{
			...
			else if (auto variable = dynamic_cast<VariableDeclaration const*>(decl))
			{
				...
				{
					unsigned stackDiff = unsigned(_assembly.stackHeight()) - m_context.baseStackOffsetOfVariable(*variable);
		...
		else
		{
			...
			solAssert(variable->type()->sizeOnStack() == 1, "");
			unsigned stackDiff = unsigned(_assembly.stackHeight()) - m_context.baseStackOffsetOfVariable(*variable) - 1;
